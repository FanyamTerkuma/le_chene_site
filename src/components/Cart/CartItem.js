...
const CartItem = ({ product, selectedItems, setselectedItems, index }) => {
  const dispatch = useDispatch();

  const checkForItem = () => {
    let exists = selectedItems.find(
      (item) =>
        item.id === product.id &&
        item.currentColor === product.currentColor &&
  ...
  const toggleCheck = () => {
    if (checkForItem()) {
      let foundIndex = selectedItems.findIndex(
        (item) =>
          item.id === product.id &&
          item.currentColor === product.currentColor &&
          item.currentSize === product.currentSize
      ...
      setselectedItems(selectedItems.filter((_, i) => i !== foundIndex));
    } else {
