...
import { addToCart, removeFromCart, toggleWishlist } from '../../reduxKit/reducer';

const productColors = ['#fbaf37', '#4b5563', '#cd4c19', '#e03fb4'];
	...
	return (
		<div className='w-full bg-white mt-5'>
			<div className='grid grid-cols-2 gap-5 max-w-5xl mx-auto'>
				<img src={product.image} alt='product' className='w-[90%] h-[35rem] rounded-md shadow-lg' />
				...
				<div className='space-y-4'>
					...
					<ProductProperties selectedProduct={selectedProduct} setselectedProduct={setselectedProduct} />
					...
					<div className='border border-[#26867B] rounded-md p-3 space-y-3'>
						<div className='flex items-center space-x-1 text-xs text-gray-800'>
							...
							<MdOutlineLocationOn />
							<p>No 2 Lateef Agbaje street, Ago Palace Way, Lagos State, Nigeria.</p>
						</div>

						<PropertyHeader title={'Estimated Delivery'} val={'02 Aug 2022'} extraCN='text-xs' />
					</div>

					<div className='p-3 space-x-2 flex items-center border border-[#26867B] text-[#26867B] rounded-md'>
						{/* commented this out so
            <img src={safeTransactionLogo} alt='safe transaction' className='h-6 object-contain' /> */}
						<p className='text-sm text-gray-700'>75-Day Buyer Protection Money back guarantee</p>
					</div>
					...
					<div className='flex items-center space-x-3'>
						{checkCart() ? (
							...
							<Button onClick={handleAddToCart} title='Add to cart' width='w-32' />
						)}
